...
void state_publisher_init()
{
    ...
    joint_state_msg_.name.resize(m->nu + 6 * n_free);
    joint_state_msg_.position.resize(m->nu + 7 * n_free);
    joint_state_msg_.velocity.resize(m->nu + 6 * n_free);
    joint_state_msg_.effort.resize(m->nu + 6 * n_free);
    
    if (n_free > 0){
        unsigned int n_unactuated = 0;
        unsigned int joint_idx = 0;
 
        for (int i=0; i<m->njnt; i++){
            if (m->jnt_type[i] == 0){
                joint_state_msg_.name[joint_idx] = "virtual_x" + std::to_string(n_free);
                joint_state_msg_.name[joint_idx + 1] = "virtual_y" + std::to_string(n_free);
                joint_state_msg_.name[joint_idx + 2] = "virtual_z" + std::to_string(n_free);
                joint_state_msg_.name[joint_idx + 3] = "virtual_roll" + std::to_string(n_free);
                joint_state_msg_.name[joint_idx + 4] = "virtual_pitch" + std::to_string(n_free);
                joint_state_msg_.name[joint_idx + 5] = "virtual_yaw" + std::to_string(n_free);

                joint_idx += 6;
                n_unactuated++;
            }
            else{
                std::string buffer(m->names + m->name_actuatoradr[i-n_unactuated]);                
                joint_state_msg_.name[joint_idx] = buffer;
                joint_idx++;
            }
        }        
    }
    else if ((m->jnt_type[0] == 2) || (m->jnt_type[0] == 3))
    {
        ...
        for (int i = 0; i < m->njnt; i++)
        {
            ...
            joint_state_msg_.position[i] = m->key_qpos[i];
        }
    }
    ...
    base_transformation_msg_.position.resize(7);
    base_transformation_msg_.velocity.resize(6);
    base_transformation_msg_.effort.resize(6);
...
void state_publisher()
{
    ...
    if (m->njnt != m->nu){ //floating 
        unsigned int n_unactuated = 0;
        unsigned int joint_idx = 0, joint_p_idx=0;

        for (int i=0; i<m->njnt; i++){
            if (m->jnt_type[i] == 0){
                for (int j=0; j<6; j++){
                    joint_state_msg_.velocity[joint_idx + j] = d->qvel[joint_idx + j];
                    joint_state_msg_.effort[joint_idx + j] = d->qacc[joint_idx + j];
                }
                for (int j=0; j<7; j++)
                    joint_state_msg_.position[joint_p_idx + j] = d->qpos[joint_p_idx + j];
                ...
                joint_state_msg_.position[joint_p_idx] = d->qpos[joint_p_idx];
                joint_state_msg_.velocity[joint_idx] = d->qvel[joint_idx];
                joint_state_msg_.effort[joint_idx] = d->qacc[joint_idx];
    ...
    // cout << "mine: ";
    // for (int i=0; i< 36; i++){
    //     cout << joint_state_msg_.position[i] << " ";
    // }
    // cout << endl;
    ...
    //     for (int i = 0; i < m->nu; i++)
    //     {
    //         joint_state_msg_.position[i + 7] = d->qpos[i + 7];
    ...
    //     }
    //     joint_state_msg_.position[6] = d->qpos[3];
    // }
    // cout << "Jun: ";
    // for (int i=0; i< 36; i++){
    //     cout << joint_state_msg_.position[i] << " ";
    ...
    int base_id;
    base_id = mj_name2id(m, 1, "l_husky");

    for(int i = 0; i < 3; i++)
    {
        base_transformation_msg_.position[i] = d->xpos[base_id * 3 + i];
    }

    for(int i = 0; i < 4; i++)
    {
        base_transformation_msg_.position[i + 3] = d->xquat[base_id * 4 + i];
    }

    mjtNum bvel[6];
    mj_objectVelocity(m, d, 1, base_id, bvel, false);

    for (int i=0; i<3; i++){
        
       base_transformation_msg_.velocity[i] = bvel[i+3];
       base_transformation_msg_.velocity[i+3] = bvel[i];
       // base_transformation_msg_.effort[i] = d->qacc[base_id * 3 + i];
    }
    
    base_state_pub.publish(base_transformation_msg_);
}
...
void mycontroller(const mjModel *m, mjData *d)
{
    ...
    if (settings.run)
    {
        if (ros_sim_started)
        {
            ...
            // if(dis_check == true)
            // {
            //     mjtNum force[3] = {0, 0.0005, 0.0};
            //     mjtNum torque[3] = {0.0, 0.0, 0.0};
            //     mjtNum point[3] = {0.0, 0.0, 0.0};

            //     mj_applyFT(m, d,force, torque, point, 22, d->qfrc_applied);
